...
class DynamicVoiceSampleCreator:
    ...
    def step7_apply_eq(self, input_path: str) -> str:
        ...
        print("[STEP 7] Applying EQ filter (highpass 100Hz, lowpass 10000Hz)...")
    ...
    def step8_apply_nlm(self, input_path: str) -> str:
        ...
        print("[STEP 8] Applying SOTA NLM noise reduction...")
        
        # SOTA scaling for anlmdn (FFmpeg default is 0.001)
        # 1-100 scale maps to 0.0001 - 0.01 (cleaner results)
        raw_strength = getattr(self.config.noise_reduction, 'strength', 50)
        strength = max(0.0001, min(0.01, raw_strength / 10000.0))
    ...
    def step9_final_loudnorm(self, input_path: str) -> str:
        ...
        if result.returncode != 0:
            print(f"[STEP 9] Warning: Loudnorm failed, using original")
            return input_path
