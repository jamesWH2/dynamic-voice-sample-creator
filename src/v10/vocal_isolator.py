...
class VocalIsolator:
    ...
    def isolate(self, input_path: str) -> str:
        ...
        # Use audio-separator CLI (from venv if available)
        venv_audio_sep = Path(__file__).parent.parent / "venv" / "bin" / "audio-separator"
        audio_sep_cmd = str(venv_audio_sep) if venv_audio_sep.exists() else "audio-separator"
